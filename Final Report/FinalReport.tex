...
There is simply too large a volume of news available to read online, and each piece of news itself could have multiple different viewpoints depending on where you read it from.

The aim of this project was to create a news aggregator, NewSumm, that summarises clusters of articles for users, thus allowing a reader to save time by covering multiple viewpoints on a piece of news within the scope of one short article. 

NewSumm also aimed to allow customisability of summary sources for users, so that they can pick and choose which sources they read and tailor their experience.

Whilst an ambitious initial plan to implement an algorithm for abstractive summarisation may have gone awry, NewSumm started producing more than competent summaries using a specially modified version of LexRank. 

User feedback for NewSumm is largely positive so far, but not without potential areas for improvement.
...
The primary aims will be largely satisfied by the final product, as proven by feedback from potential users through an extensive survey.

Users will speak of the clean and slick design that NewSumm has, and will be found to be captured by the concept of being able to "gather all sources on a particular topic at once", thus "saving time".

Going side by side with this evaluation of the fundamentals of the system itself will be a rigorous analysis of the summaries produced to establish whether the system can be affected by the various political biases of its sources. The analysis will showcase a potential vulnerability in the summariser, in that articles from sources that are considered to be purely factual can be marginalised, which in turn affects the success of the secondary aim of providing a near guarantee that all news produced is trustworthy. 
...
The following survey was conducted using SurveyMonkey \cite{surveymonkey}, and was answered by 26 respondents. The survey was split into three key sections: Labelling, Clustering and Summarisation.
